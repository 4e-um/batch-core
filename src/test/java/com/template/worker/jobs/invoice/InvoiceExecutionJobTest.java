...
class InvoiceExecutionJobTest {
    ...
    @Autowired JobLauncherTestUtils jobLauncherTestUtils; // Batch Job을 테스트용으로 실행해주는 유틸

    @Autowired
    @Qualifier("invoiceJob") // 실제 실행할 Job Bean
    Job invoiceExecutionJob;

    @Autowired JdbcTemplate jdbcTemplate; // DB 결과 검증용
    ...
    void invoiceExecutionJobTest() throws Exception {
        ...
        Integer count =
                jdbcTemplate.queryForObject(
                        "SELECT COUNT(*) FROM invoice WHERE inv_month = '202508'", Integer.class);

        assertThat(count).isGreaterThan(0);
    }

    /**
     * 테스트용 JobParameters 생성 - invMonth : Job/Reader에서 사용하는 핵심 파라미터 - run.id : 매 실행마다 다른 JobInstance
     * 생성을 위한 값
     */
